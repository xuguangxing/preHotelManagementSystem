layui.config({
        ...
        flow.load({
            ...
            , done: function (page, next) { //执行下一页的回调
                ...
                $.get({
                    ...
                    url: "http://localhost:9001/roomType/queryAllRoomRType?page="+page+"&limit=10",
    ...
    function getAllHotelFloor() {
        $.ajax({
            url: "http://localhost:9001/floor/queryAllFloor?page=1&limit=10",
           /* xhrFields:{withCredentials: true},*/
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    $.each(res.data, function (index, floor) {
                        ...
                        if (index % 2 == 0) {
                            ...
                            getIndexFloorHtml += '<p class="temp-title-cn"><span></span>酒店' + (index + 1) + '楼<span></span></p>\n';
                            ...
                            $.each(floor.roomList, function (index, room) {
                                ...
                                getIndexFloorHtml += '<img src="' + room.coverImg + '" class="store-list-cover">\n';
                                getIndexFloorHtml += '<h2 class="layui-elip">' + room.roomName + '</h2>\n';
                                getIndexFloorHtml += '<p class="price"> <span title="金额"> ￥' + room.memberPrice + ' </span> <span title="房号" style="color:  #fff;background: #0e88cc;padding: 3px;text-align: center;border: 1px solid #4cffb3;font-size: 13px;"> NO.' + room.roomNumber + ' </span></p>\n';
                                getIndexFloorHtml += '</a>\n';
                            ...
                            <!--循环开始2 start-->
                            getIndexFloorHtml += '<div class="temp-normal">\n';
                            getIndexFloorHtml += '<div class="layui-container">\n';
                            getIndexFloorHtml += '<p class="temp-title-cn"> <span></span> <a href="">酒店' + (index + 1) + '楼</a> <span></span> </p>\n';
                            getIndexFloorHtml += '<div class="layui-row layui-col-space20 shoplist">\n';
                            $.each(floor.roomList, function (index, room) {
                                getIndexFloorHtml += '<div data-id="' + room.id + '" class="layui-col-xs12 layui-col-sm6 layui-col-md4 layui-col-lg3"> \n';
                                getIndexFloorHtml += '<a class="template store-list-box fly-case-active" href="JavaScript:void(0);" data-type="toRoomInfo">\n';
                                getIndexFloorHtml += '<img src="' + room.coverImg + '" class="store-list-cover">\n';
                                getIndexFloorHtml += '<h2 class="layui-elip">' + room.roomName + '</h2>\n';
                                getIndexFloorHtml += '<div> <label class="layui-badge-rim store-list-pay"> ￥' + room.memberPrice + ' </label>\n';
                                getIndexFloorHtml += '<div class="store-list-colorbar" title=""> <span class="store-color-bar" style="color:  #fff;background: #cc9812;padding: 3px;text-align: center;">NO.' + room.roomNumber + '</span>\n';
                                getIndexFloorHtml += '</div>\n';
                                getIndexFloorHtml += '</div>\n';
                                getIndexFloorHtml += '</a>\n';
                                getIndexFloorHtml += '</div>\n';
